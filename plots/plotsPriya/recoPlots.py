...
for s in sample:
    sname = s.name
    dl1 = ROOT.TH1F('1stmuon dxy','Impact Parameter of 1st muon',50,0.0,0.5)
    dl2 = ROOT.TH1F('2ndmuon dxy','Impact Parameter of 1st muon',50,0.0,0.5)
    d2 = ROOT.TH2F('dxy','Impact Parameters of leptons in dilepton state (13 TeV); first lepton d0[cm]; 2nd lepton d0[cm]',50,0.0,0.5,50,0.0,0.5)
    r = s.treeReader( \
    ...
    while r.run():
        i += 1
        pass
    plotl1 = Plot.fromHisto(name = "1st muon dxy_%s_50"%sname, histos = [[dl1]], texX = "dxy of 1st muon (cm)", texY = "Number of events" )
    plotl2 = Plot.fromHisto(name = "2nd muon dxy_%s_50"%sname, histos = [[dl2]], texX = "dxy of 2nd muon(cm)", texY = "Number of events" )
    plot1 = Plot2D.fromHisto(name = "dxy of Muons_%s_2D_50"%sname, histos = [[d2]], texX = "dxy 1st muon (cm)", texY = "dxy 2nd muon(cm)" )
    plotting.draw(plotl1, plot_directory = plot_directory, logX = False, logY = True, sorting = False, ratio = None  )
    plotting.draw(plotl2, plot_directory = plot_directory, logX = False, logY = True, sorting = False, ratio = None  )
    plotting.draw2D(plot1, plot_directory = plot_directory, logX = False, logY = False,logZ = True )
