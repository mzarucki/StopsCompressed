...
if args.eos and "2016" in args.era:
    ...
    from StopsCompressed.samples.nanoTuples_RunUL16_postProcessed import *
    from StopsCompressed.samples.nanoTuples_RunUL16APV_postProcessed import *
    data_samples = [Run2016postVFP, Run2016preVFP]
    samples = []
    signals = []
    ...
    data_samples = [Run2016preVFP, Run2016postVFP]
    signals = []
    #if args.reweightPU:
...
for index, mode in enumerate(allModes):
	...
	if args.era == "Run2016":
		...
		data_samples[0].style          = styles.errorStyle(ROOT.kRed)
		data_samples[1].style          = styles.errorStyle(ROOT.kBlack)
	else:
	...
	stack_ = Stack( data_samples[0], data_samples[1] ) 
	...
	if args.small:
		for sample in samples + [data_samples] + signals:
