...
argParser.add_argument('--targetDir',          action='store',      default='v07')
...
plot_directory = os.path.join(plot_directory,'reco', args.targetDir, 'signal_bkg', 'wo_dxy0p1_dimuon')
...
Plot.setDefaults(stack = stack, weight=staticmethod( weight_ ),selectionString = 'Sum$(Muon_pt>5&&abs(Muon_eta)<2.4&&Muon_miniPFRelIso_all<.2&&(abs(Muon_dxy)>0.1))==2')
#Plot2D.setDefaults(stack = stack,selectionString = 'Sum$(Muon_pt>5&&abs(Muon_eta)<2.4&&Muon_miniPFRelIso_all<.2&&(abs(Muon_dxy)>0.1))==2')
...
plots2D = []
plots2D.append(Plot2D( name = "TTLep_di_muon_dxy", texX = "dxy of 1st muon (cm)", texY = "dxy of 2nd muon (cm)", stack = Stack([TTLep_pow]), attribute =(lambda event, sample: event.mu1,lambda event, sample: event.mu2), binning=[100,0.0,10.0,100,0.0,10.0], weight= weight_ ,selectionString = 'Sum$(Muon_pt>5&&abs(Muon_eta)<2.4&&Muon_miniPFRelIso_all<.2&&(abs(Muon_dxy)>0.1))==2'))
plots2D.append(Plot2D( name = "TTLep_dimuon_1st_dxy_vs_pt", texX = "dxy of 1st muon (cm)", texY = "pT of 1st muon(GeV)", stack = Stack([TTLep_pow]), attribute =(lambda event, sample: event.mu1,lambda event, sample: event.mu1pt), binning=[100,0.0,10.0,120,0.0,60.0], weight= weight_ ,selectionString = 'Sum$(Muon_pt>5&&abs(Muon_eta)<2.4&&Muon_miniPFRelIso_all<.2&&(abs(Muon_dxy)>0.1))==2'))
#
#plot = plots + plots2D
