...
def combineSFs(baseHistName, updateHistName, updatePtRange, file_dir = './', savedir = './', drawOriginalSFs = False, verbose = False):
    ...
    if not updatePtRange[0] < updatePtRange[1]:
        ...
        sys.exit()
    #change it to check ybins, as pt is ybinned in electron channels 
    if not all(y in selectedHistDict[updateHistName]['binning']['ybins'] for y in updatePtRange):
        print "updatePtRange %s must coincide with binning %s of histogram that updates the SFs. Exiting."%(updatePtRange, selectedHistDict[updateHistName]['binning']['ybins'])
        sys.exit() 
    
    #change these to get ybins for histograms
    updatePtBinsAll = selectedHistDict[updateHistName]['binning']['ybins']
    basePtBinsAll   = selectedHistDict[baseHistName]['binning']['ybins']
    ...
    print "base pt delta: ", basePtBinsLowerDelta, "update pt delta: ", updatePtBinsDelta
    ...
    combinedHistDict['binning']['ybins'] = combinedPtBins
    combinedHistDict['binning']['ny'] = ny
    
    # filling combined histogram
    if combinedHistDict['dim'] == '2D': # 2D SF plot
        baseEtaBins = selectedHistDict[baseHistName]['binning']['xbins'] # currently keeping base eta binning #NOTE: equivalent to selectedHistDict[baseHistName]['hist'].GetYaxis().GetXbins()).GetArray()
	...
	print "Eta bins delta: ",positiveEtaBinsDelta
        ...
        SF = ROOT.TH2F(combinedHistName, combinedHistName, nx,combinedEtaBinsArray,ny, combinedPtBinsArray)
        ...
        for xbin in range(1, nx+1):
            ...
            for ybin in range(1, ny+1):
                if combinedPtBins[ybin-1] < updatePtRange[0]:
                    binContent = selectedHistDict[baseHistName]['hist'].GetBinContent(xbin+positiveEtaBinsDelta, ybin)
                    binError   = selectedHistDict[baseHistName]['hist'].GetBinError(xbin+positiveEtaBinsDelta, ybin)
                # updated values
                elif combinedPtBins[ybin-1] >= updatePtRange[0] and combinedPtBins[ybin-1] < updatePtRange[1]: # NOTE: does not include the bin starting from the upper edge
                    if selectedHistDict[updateHistName]['dim'] == "2D":
                        ...
                        xbinFix    = selectedHistDict[updateHistName]['hist'].GetXaxis().FindBin(selectedHistDict[baseHistName]['hist'].GetXaxis().GetBinCenter(xbin+positiveEtaBinsDelta)) # NOTE: if eta bins are not identical
                ...
                elif combinedPtBins[ybin-1] >= updatePtRange[1]:
                    binContent = selectedHistDict[baseHistName]['hist'].GetBinContent(xbin+positiveEtaBinsDelta, ybin-updatePtBinsDelta)
                    binError   = selectedHistDict[baseHistName]['hist'].GetBinError(xbin+positiveEtaBinsDelta, ybin-updatePtBinsDelta)
                else:
...
if __name__ == '__main__':
    savedir = "/mnt/hephy/cms/priya.hussain/www/StopsCompressed/leptonSFs/2016/cent/combinedSFs"
    ...
    combineSFs(baseHistName = 'el_VetoWP_2D_cent',  updateHistName = 'el_VetoWP_2D_priv',  updatePtRange = [5, 10], file_dir = '/scratch/priya.hussain/StopsCompressed/results/2016_80X_v5/finalplots/mod/', savedir = savedir, drawOriginalSFs = True, verbose = True)
