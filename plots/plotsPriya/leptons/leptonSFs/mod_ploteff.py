...
f = TFile("/scratch/priya.hussain/StopsCompressed/results/%s/fits/%s_result_MC_%s%s.root"%(datatag,flavor,stage,suffix))
effcntZMC = TEfficiency(f.Get("effcnt"))
efffitZMC = TEfficiency(f.Get("efffit"))
f.Close()
f = TFile("/scratch/priya.hussain/StopsCompressed/results/%s/fits/%s_result_Data_%s%s.root"%(datatag,flavor,stage,suffix))
...
savedir = "/mnt/hephy/cms/priya.hussain/www/StopsCompressed/TnP/%s/finalplots/%s"%(datatag,stage)
makeDir(savedir)
makeDir("/scratch/priya.hussain/StopsCompressed/results/%s/finalplots"%datatag)

c1.SaveAs("/scratch/priya.hussain/StopsCompressed/results/%s/finalplots/%s_eff_%s_%s.png"%(datatag,flavor,stage,etabin))
c1.SaveAs("%s/%s_eff_%s_%s.png"%(savedir,flavor,stage,etabin))
fout = TFile("/scratch/priya.hussain/StopsCompressed/results/%s/finalplots/%s_eff_%s_%s.root"%(datatag,flavor,stage,etabin),"RECREATE")
c1c = c1.Clone()
c1c.Write()
fout.Close()
...
c2.SaveAs("/scratch/priya.hussain/StopsCompressed/results/%s/finalplots/%s_SF_%s_%s.png"%(datatag,flavor,stage,etabin))
c2.SaveAs("%s/%s_SF_%s_%s.png"%(savedir,flavor,stage,etabin))
fout = TFile("/scratch/priya.hussain/StopsCompressed/results/%s/finalplots/%s_SF_%s_%s.root"%(datatag,flavor,stage,etabin),"RECREATE")
c2c = c2.Clone()
c2c.Write()
fout.Close()

fsfout = TFile("/scratch/priya.hussain/StopsCompressed/results/%s/finalplots/hephy_scale_factors.root"%datatag,"update")
