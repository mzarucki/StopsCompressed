...
plotsDict = {
   ...
   "nISRJets"         :{'var':"nISRJets",             "bins":[10,0,10],       "decor":{"title":"# of ISR Jets",                 "x":"N(ISR Jets p_{T} > 100 GeV)",             "y":"Events", 'log':[0,logy,0]}},
   ...
   "nBJets"           :{'var':"nBTag",                "bins":[10,0,10],       "decor":{"title":"# of B-Tagged Jets",            "x":"N(B Jets)",                               "y":"Events", 'log':[0,logy,0]}},
   "nSoftBJets"       :{'var':"nSoftBJets",           "bins":[10,0,10],       "decor":{"title":"# of Soft B-Tagged Jets",       "x":"N(B Jets p_{T} < 60 GeV)",                "y":"Events", 'log':[0,logy,0]}},
   "nHardBJets"       :{'var':"nHardBJets",           "bins":[10,0,10],       "decor":{"title":"# of Hard B-Tagged Jets",       "x":"N(B Jets p_{T} > 60 GeV)",                "y":"Events", 'log':[0,logy,0]}},
    ...
    if nMinus1:
        for p in plotList:
            ...
            # CT2 fully correlated with MET and ISR jet pT
                ...
                cut_str[dataMC]['nMinus1'][p] = "&&".join([x for x in cut_str[dataMC]['full'].split('&&') if plotsDict[p]['var'] not in x and 'CT2' not in x and 'HT' not in x])
            elif p == "MET":
...
for p in plotList:
    ...
    for i, sig in enumerate(sigList):
        if "T2tt" in sig:
            ...
            mu = int(sig.split('_')[1])
            M1 = int(sig.split('_')[2])
    ...
    if isDataPlot:
        ...
        drawCMSHeader(lumi = lumi_fb, preliminary = "Private Work")
