# Script for making data/MC plots
...
parser.add_argument("--region",    dest = "region",    help = "Analysis region", type = str, default = "presel", choices = ["presel", "CR1", "CR2"]) # NOTE: only preselection and CRs as choice due to blinding purposes
parser.add_argument("--getData",   dest = "getData",   help = "Get data samples", type = int, default = 1)
parser.add_argument("--getSignal", dest = "getSignal", help = "Get data samples", type = int, default = 1)
...
parser.add_argument("--triggerSelection", dest = "triggerSelection", help = "Trigger selection", type = str, choices = ['PureMET', 'SoftMuHardJetMET', 'PureMET_OR_SoftMuHardJetMET'], default = None)
...
triggerSelection = args.triggerSelection
...
print "Creating data/MC plots %s in %s"%(args.plots, args.region)
print makeDoubleLine()
...
triggerSelections = {
    "PureMET": "HLT_PFMET120_PFMHT120_IDTight",
    "SoftMuHardJetMET": "HLT_Mu3er1p5_PFJet100er2p5_PFMET80_PFMHT80_IDTight"
}

triggerSelections["PureMET_OR_SoftMuHardJetMET"] = "||".join(triggerSelections.values())

for trig in triggerSelections:
    triggerSelections[trig] = "Alt$(%s,0)"%triggerSelections[trig] # NOTE: protection if branch not present
...
for dataMC in ["Data", "MC"]: # DataMC
    ...
    if triggerSelection:
        cut_str[dataMC]['full'] += "&& (%s)"%triggerSelections[triggerSelection]
...
for p in plotList:
    for dataMC in ["Data", "MC"]:
        if nMinus1:
            cut_string[dataMC] = cut_str['MC']['nMinus1'][p]
        else:
            cut_string[dataMC] = cut_str['MC']['full']

    bkgHists = []
    # MC
    for samp in sampleList:
        hists[samp][p] = getPlotFromChain(samples[samp].chain, plotsDict[p]['var'], plotsDict[p]['bins'], cut_string["MC"], weight = weight_str["MC"], addOverFlowBin = addOverFlowBin, binningIsExplicit = binningIsExplicit, variableBinning = variableBinning, uniqueName = False)
    ...
    # Data
    if getData:
        hists['Data'][p] = getPlotFromChain(Data.chain, plotsDict[p]['var'], plotsDict[p]['bins'], cut_string["Data"], weight = weight_str["Data"], addOverFlowBin = addOverFlowBin, binningIsExplicit = binningIsExplicit, variableBinning = variableBinning, uniqueName = False)
    ...
    if getSignal:
        ...
        for i, sig in enumerate(sigList):
            ...
            hists[sig][p] = getPlotFromChain(selSignals[i].chain, plotsDict[p]['var'], plotsDict[p]['bins'], cut_string["MC"], weight = "(%s * %s)"%(weight_str["MC"], genEff), addOverFlowBin = addOverFlowBin, binningIsExplicit = binningIsExplicit, variableBinning = variableBinning, uniqueName = False)
    ...
    if save: #web address: https://mzarucki.web.cern.ch/
        ...
        savedir = "/eos/user/m/mzarucki/www/%s/dataMC/%s/%s/%s"%(tag, sensitivityStudyName, args.region, channel)
        ...
        if triggerSelection:
            savedir += "/triggerSelection"
            suffix += "_" + triggerSelection
